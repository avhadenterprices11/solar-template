    ...
    <section className="py-24 bg-slate-50 dark:bg-slate-900/50">
      <div className="container mx-auto px-6">
        <div className="mb-16 space-y-4">
          <h2 className="text-sm font-black text-amber-500 uppercase tracking-widest leading-none">Engineering Deep-Dive</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {analysisItems.map((item) => (
            <div 
              key={item.id} 
              className="bg-white dark:bg-slate-950 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-black/5 flex flex-col h-full group hover:border-amber-500/50 transition-all duration-500"
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <h4 className="text-xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter transition-colors group-hover:text-amber-500">
                  {item.title}
                </h4>
              </div>
              ...
              <ul className="space-y-4 flex-grow">
                {item.data?.map((point, index) => (
                  <li key={index} className="flex gap-4 group/point">
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-500 mt-2 shrink-0 transition-transform group-hover/point:scale-150" />
