...
export function ProjectImpact({ impact }: ProjectImpactProps) {
  ...
  return (
    <section className="py-24 bg-slate-900 rounded-[3.5rem] border border-white/5 overflow-hidden relative">
      {/* Background Accent */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-brand-green/10 blur-[120px] rounded-full" />
      
      <div className="container mx-auto px-12 relative z-10">
        <div className="max-w-3xl space-y-6 mb-16">
          <h2 className="text-sm font-bold text-brand-green uppercase tracking-[0.2em] leading-none">Evidence of Success</h2>
          ...
          {impact.map((text, idx) => {
            ...
            return (
              <div key={idx} className="p-8 bg-white/5 border border-white/10 rounded-3xl hover:bg-white/10 transition-all group">
                <div className="w-12 h-12 bg-brand-green text-brand-navy rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                  <Icon size={24} />
                </div>
