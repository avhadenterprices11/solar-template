    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          {scope.map((item, idx) => (
            <div key={idx} className="flex items-start gap-4 p-5 bg-brand-gray dark:bg-slate-900 rounded-2xl border border-brand-gray dark:border-slate-800 group hover:border-brand-green/30 transition-all">
               <div className="p-2 bg-white dark:bg-brand-navy text-brand-green rounded-lg shadow-sm">
                  <CheckCircle size={18} />
               </div>
        ...
        <div className="relative space-y-8 before:absolute before:left-[19px] before:top-4 before:bottom-4 before:w-px before:bg-slate-200 dark:before:bg-slate-800">
           {approach.map((step, idx) => (
             <div key={idx} className="relative pl-12 group">
                <div className="absolute left-0 top-0 w-10 h-10 bg-white dark:bg-brand-navy border-2 border-slate-200 dark:border-slate-800 rounded-full flex items-center justify-center text-xs font-black text-slate-400 group-hover:border-brand-green group-hover:text-brand-green transition-colors z-10">
                   {idx + 1}
                </div>
